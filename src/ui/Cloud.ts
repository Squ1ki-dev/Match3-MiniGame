...
export class Cloud extends Container {
    ...
    public async playFormAnimation(duration = 1) {
        ...
        const circleAnimations = this.circles.map(circle => {
            ...
            return gsap.timeline()
                .to(circle.scale, { x: 1, y: 1, duration, ease }, 0)
                .to(circle.pivot, { x: 0, y: 0, duration, ease }, 0);
        });
    
        this.top.pivot.y = this.top.y;
        this.bottom.pivot.y = this.bottom.y;
    
        const topBottomAnimations = gsap.timeline()
            .to(this.top.pivot, { y: 0, duration, ease }, 0)
            .to(this.bottom.pivot, { y: 0, duration, ease }, 0);
        ...
        const baseAnimation = gsap.timeline()
            .to(this.base, { alpha: 1, duration, ease }, 0)
            .to(this.base, { width, height, duration, ease });
    
        // Combine all animations
        await gsap.timeline()
            .add(circleAnimations)
            .add(topBottomAnimations, 0)
            .add(baseAnimation, 0);
    }    
